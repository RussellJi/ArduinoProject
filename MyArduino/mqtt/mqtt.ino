...
char dataTemplate[] = "{\"temperature\":%.2f,\"humidity\":%.2f}"; //信息模板
...
void clientReconnect()
{
  while (!client.connected()) //再重连客户端
  {
    ...
    if (client.connect(mqtt_devid, mqtt_pubid, mqtt_password))
    {
      ...
      bool rec = client.subscribe("topic",0)
      if(rec == false)
      {
...
void setup()
{
  ...
  clientReconnect(); //客户端连接到指定的产品的指定设备.同时输入鉴权信息
  client.setCallback(callback);                          //设置好客户端收到信息时的回调
